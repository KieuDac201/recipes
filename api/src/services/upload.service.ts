...
export const uploadImageToCloudinary = (fileBuffer: Buffer, folder: string): Promise<UploadResponse> => {
...
export const cleanOrphanedImages = async (): Promise<void> => {
    try {
        ...
        if (toHealIds.length > 0) {
            ...
            console.log(`[Cron Job] Healed & kept ${toHealIds.length} images that exist in Database:`, toHealIds);
        }
        // Xóa các ảnh mồ côi thực sự
        if (toDeleteIds.length > 0) {
            await cloudinary.api.delete_resources(toDeleteIds);
            console.log(`[Cron Job] Successfully deleted ${toDeleteIds.length} truly orphaned images:`, toDeleteIds);
        }
    } catch (error) {
