...
const getMyRecipes = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const {
      ...
      search,
      status,
    } = req.query as unknown as GetMyRecipesQuery
    const { id: authorId } = req.user as { id: number }
    const { recipes, totalPage } = await RecipeService.getMyRecipes(
      limit,
      currentPage,
      search,
      status,
...
const getAdminRecipes = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const {
      ...
      search,
      status,
    } = req.query as unknown as GetAdminRecipesQuery
    const { recipes, totalPage, totalCount } = await RecipeService.getAdminRecipes(
      limit,
      currentPage,
      search,
      status
