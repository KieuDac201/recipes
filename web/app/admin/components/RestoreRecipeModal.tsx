...
interface RestoreRecipeModalProps {
  recipe: Recipe | null;
  isLoading: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export default function RestoreRecipeModal({
  recipe,
  isLoading,
  onClose,
  onConfirm,
}: RestoreRecipeModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-[28px] max-w-md w-full p-6 md:p-8 shadow-2xl border border-[#efeeea] animate-in zoom-in-95 duration-200 relative">
        <div className="w-14 h-14 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center mb-5 mx-auto border border-emerald-100">
          <span className="material-symbols-outlined text-[32px]">restore_from_trash</span>
        </div>
        ...
        <p className="text-sm text-[#584140] text-center mb-6 leading-relaxed">
          Bạn có chắc chắn muốn khôi phục công thức món ăn này? Công thức sẽ được đưa trở lại danh sách hoạt động trên hệ thống.
        </p>
